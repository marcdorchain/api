...
import { Route, RoutePath } from '../../models/route.interface';
...
import { arrayEquals, objectEquals } from '../../utils/utils';
...
@Component({
    ...
    imports: [CommonModule, MatProgressSpinnerModule, MatTabsModule, MatIconModule, MatButtonModule, MatTooltipModule, MatFormFieldModule, MatInputModule, FormsModule, ReactiveFormsModule, CdkDropList, CdkDrag, FilterFormComponent, FilterSelectComponent, MatSidenavModule, MatExpansionModule, MatDividerModule, MatCardModule, MatListModule, MatSlideToggleModule, MatChipsModule]
})
export class RouteEditorComponent implements OnInit{

  public methodColorMap = {
    "POST": "chip-green",
    "GET": "chip-blue",
    "PUT": "chip-amber",
    "PATCH": "patch-turqoise",
    "DELETE": "chip-red",
    "HEAD": "chip-purple",
    "OPTIONS": "chip-navy",
    "TRACE": "chip-black"
  ...
  form = new FormGroup({
    ...
    paths: new FormArray<FormControl<boolean|null>>([]),
    settings: new FormGroup({
      alwaysAllowHEAD: new FormControl<boolean>(true, {nonNullable: true}),
      alwaysAllowOPTIONS: new FormControl<boolean>(true, {nonNullable: true})
    })
  });
  ...
  private intialise(route: Route){
    ...
    this.form.controls["paths"].clear();
    this.form.reset({
    ...
    if(route.paths){
      route.paths.forEach((path) => this.form.controls["paths"].push(new FormControl<boolean>(path.active, Validators.required)));
    }
  }
  ...
  hasChanges(): boolean {
    return this.route?.endpoint != this.getFieldValue('endpoint')
         ...
         || !objectEquals(this.route?.settings, this.form.controls['settings'].value)
         || !arrayEquals(this.route?.paths?.map((path) => path.active) || [], this.form.controls['paths'].value);
  }
  ...
  saveChanges(){
    ...
    req.settings = this.form.controls['settings'].getRawValue();
    if(req.paths){
      let i = 0;
      while(i < req.paths.length){
        req.paths[i].active = this.form.controls['paths'].at(i).value || false;
        i++;
      }
    }
